...
import { Story, Outline, FrameworkId } from '../../types'
      ...
      <div className="flex flex-col gap-4">
        <div className="flex flex-col gap-1.5">
          ...
          <input
            ...
            className="h-9 px-3 rounded-[var(--radius-md)] border border-[var(--border)] text-sm text-[var(--text-primary)] placeholder:text-[var(--text-muted)] focus:border-[var(--border-active)] outline-none transition-colors duration-[var(--dur-fast)]"
          ...
          <div className="flex gap-2 flex-wrap">
            {COLOURS.map(c => (
              <button
                ...
                onClick={() => setColour(c)}
                className={clsx(
                  'w-7 h-7 rounded-full transition-all duration-[var(--dur-fast)]',
...
function NewOutlineModal({ storyId, open, onClose }: {
  storyId: string; open: boolean; onClose: () => void
}) {
  const [title, setTitle] = useState('')
  const [frameworkId, setFrameworkId] = useState<FrameworkId>(3)
  const addOutline = useBoundStore(s => s.addOutline)
  const setActiveOutline = useBoundStore(s => s.setActiveOutline)

  const submit = () => {
    if (!title.trim()) return
    const outline = addOutline(storyId, title.trim(), frameworkId)
    setActiveOutline(outline.id)
    setTitle('')
    onClose()
  }
          ...
          <div className="flex flex-col gap-1 max-h-64 overflow-y-auto pr-1">
            {FRAMEWORKS.map(fw => (
              ...
              >
                ...
                <span className="text-[11px] text-[var(--accent-orange)] font-[family-name:var(--font-heading)] tracking-wide flex-shrink-0">
                  {fw.level}
                </span>
              </button>
            ))}
...
// ── Outline Row ─────────────────────────────────────────────────
function OutlineRow({ outline }: { outline: Outline }) {
  const { activeOutlineId } = useUI()
  const setActiveOutline = useBoundStore(s => s.setActiveOutline)
  const active = activeOutlineId === outline.id

  const fwLabel = `F${outline.frameworkId}`

  return (
    <button
      onClick={() => setActiveOutline(outline.id)}
      className={clsx(
        'w-full flex items-center gap-2 h-8 pl-10 pr-3 text-xs group',
    ...
    >
      ...
      <span className="text-[10px] text-[var(--accent-orange)] font-[family-name:var(--font-heading)] tracking-wide flex-shrink-0 opacity-0 group-hover:opacity-100 transition-opacity duration-[var(--dur-fast)]">
        {fwLabel}
      </span>
    </button>
  )
...
function StoryRow({ story }: { story: Story }) {
  ...
  // 36px per child + 4px padding top/bottom
  const expandedHeight = expanded ? childCount * 32 + (childCount === 0 ? 32 : 0) : 0
  ...
  return (
    <>
      ...
      <div
        className={clsx(
          'flex items-center h-9 px-2 gap-1.5 cursor-pointer select-none group',
          'transition-colors duration-[var(--dur-fast)]',
          active
            ? 'bg-[var(--accent-teal-10)]'
            : 'hover:bg-[var(--accent-teal-10)]'
        )}
        onClick={handleClick}
      >
        {/* Label colour stripe */}
        <span
          ...
          style={{ background: story.labelColour }}
        />
        ...
        {/* Title */}
        <span className="flex-1 truncate text-[13px] font-medium text-[var(--text-primary)] min-w-0">
          {story.title}
        </span>
        ...
        <div className="flex items-center gap-0.5 opacity-0 group-hover:opacity-100 transition-opacity duration-[var(--dur-fast)]">
          <button
            onClick={e => { e.stopPropagation(); setShowNewOutline(true) }}
            className="p-1 rounded text-[var(--text-muted)] hover:text-[var(--accent-orange)] hover:bg-[var(--accent-orange-10)] transition-colors duration-[var(--dur-fast)]"
            title="New Outline"
          >
            <FilePlus size={13} />
          </button>
      ...
      >
        {outlines.map((o, i) => (
          <div
            key={o.id}
            className="animate-beat-appear"
            style={{ animationDelay: `${i * 20}ms` }}
          >
            <OutlineRow outline={o} />
          </div>
        ))}
        ...
        {notes.map((n, i) => (
          <div
            key={n.id}
            className="animate-beat-appear"
            style={{ animationDelay: `${(outlines.length + i) * 20}ms` }}
          >
...
export function Binder() {
  ...
  return (
    <>
      <aside
        className="flex flex-col border-r border-[var(--border)] flex-shrink-0"
        style={{
            ...
            {stories.length === 0 ? (
              <div className="flex flex-col items-center gap-3 py-12 px-4 text-center">
                ...
                <p className="text-xs text-[var(--text-muted)] leading-relaxed">
                  Your shelf is empty.<br />Create your first story to begin.
                </p>
                <button
                  onClick={() => setShowNewStory(true)}
                  className="text-xs text-[var(--accent-orange)] hover:text-[var(--accent-orange-dim)] transition-colors duration-[var(--dur-fast)]"
                >
                  + New Story
                </button>
              </div>
