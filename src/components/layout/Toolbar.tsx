...
import {
  ...
  Cloud, CloudOff, RefreshCw, GitBranch, LayoutGrid, Network, Table, Download,
} from 'lucide-react'
...
export function Toolbar() {
  ...
  return (
    <>
      <header className="flex items-center gap-2 px-3 flex-shrink-0 z-10 border-b border-[var(--border)]"
        style={{ height:'var(--toolbar-height)', background:'var(--bg-toolbar)' }}>

        {/* Left: binder + breadcrumb */}
            ...
            return (
              <button key={id} onClick={() => setViewMode(id)}
                className={clsx(
                  ...
                  'text-xs font-medium font-[family-name:var(--font-body)] transition-all duration-[var(--dur-fast)]',
                  active ? 'text-[var(--accent-orange)] bg-[var(--accent-orange-10)]'
        ...
        {/* Right: mode toggles + export + sync + settings */}
        <div className="flex items-center gap-0.5 flex-shrink-0" style={{ width:'200px', justifyContent:'flex-end' }}>
          {activeOutlineId && (
            <>
              <Tooltip content={splitMode ? 'Exit Split' : 'Split Mode'} placement="bottom">
                <button onClick={() => setSplitMode(!splitMode)}
                  className={clsx('p-2 rounded-[var(--radius-md)] transition-all duration-[var(--dur-fast)]',
                    splitMode ? 'text-[var(--accent-orange)] bg-[var(--accent-orange-10)]'
                      : 'text-[var(--text-muted)] hover:text-[var(--text-primary)] hover:bg-[var(--accent-teal-10)]'
                  )}>
                  <SplitSquareVertical size={18}/>
                </button>
              </Tooltip>
              <Tooltip content={focusMode ? 'Exit Focus (Esc)' : 'Focus Mode (F11)'} placement="bottom">
              ...
              <Tooltip content="Export outline" placement="bottom">
